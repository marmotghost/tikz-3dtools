...
\fi
\edef\pgfutil@tmp{\noexpand\def\noexpand\pgfutil@tmpd{\expandafter\@firstoftwo\pgfutil@tmpc}}%
\pgfutil@tmp%
...
\else
\typeout{Physical x component of #1 not stored. Setting it to 0.}
\edef\pgfmathresult{0}%
\fi
\pgfmathsmuggle\pgfmathresult\endgroup%
}%
...
\else
\typeout{Physical y component of #1 not stored. Setting it to 0.}
\edef\pgfmathresult{0}%
\fi
\pgfmathsmuggle\pgfmathresult\endgroup%
}%
...
\pgfmathtruncatemacro{\pgfutil@tmpi}{abs(\pgfutil@tmpd)>0.02?1:0}%
\ifnum\pgfutil@tmpi=0\relax
\typeout{The points approximately sit on a common line. The circumcircle cannot
be determined.}%
\else
...
\typeout{The vectors \pgfutil@tmpA, \pgfutil@tmpB\space and 
...
C/.initial={(C)},D/.initial={(D)},ex/.initial={(ex)},
...
\typeout{Points \pgfkeysvalueof{/tikz/3d/aux keys/A} and \pgfkeysvalueof{/tikz/3d/aux keys/B} are too close to each other.}%
\else
...
\typeout{Not all spheres intersect.}%
\else
