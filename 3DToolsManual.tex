...
\begin{handler}{{.add item}}
		Key handler that adds an item to a list.
\end{handler}

\begin{handler}{{.remove item}}
		Key handler that removes an item from a list.
\end{handler}

\begin{handler}{{.count}|=|\meta{macro}}
		Key handler stores the number of elements of a give list in a macro,
		which is the mandatory argument.
\end{handler}
...
\begin{handler}{{.is array}|=|\meta{list}}
		Key handler makes the current key an array consisting of \meta{list}.
		You need to use a new key for that, i.e.\ you cannot overwrite existing
		keys.
\end{handler}

\begin{handler}{{.sort numeric list}|=|{\meta{macro}}{\meta{macro}}}
		Key handler that sorts a list. The resulting sorted list is stored in
		the first argument, which has to be a macro, and the permutation is
		stored in the second argument, which needs to be a macro as well.
\end{handler}
