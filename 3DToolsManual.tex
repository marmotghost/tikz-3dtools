...
linear combinations of vectors, vector and scalar products.
...
the screen depth (see the |screendepth| function below) will become almost
mandatory at a given point. 
...
This library can be used together with the
...
$O(\phi,0,\theta)=R^d(\theta_d,\phi_d)$ from the
...
identifications $\theta=\mathtt{\textbackslash tdplotmaintheta}$ and
$\phi=\mathtt{\textbackslash tdplotmainphi}$.
...
\[
 n_\mathrm{screen}\cdot\gamma_\mathrm{lat}(\beta_\mathrm{crit})
...
the normal of the surface on the screen, $F(u,v):=\vec n_S(u,v)\cdot \vec
...
and for $\psi=0$
\begin{equation}
 n_\mathrm{screen}=\begin{pmatrix}
 	\sin(\theta)\,\sin (\phi)\\
	-\sin (\theta )\,\cos(\phi)\\
	\cos (\theta)
  \end{pmatrix}\;.
...
code could deal with these cases, the one presented below does not. Note also
...
contour in figure~\ref{fig:example}. They are a bit unilluminating and given by
...
``reasonable'' configurations, it is very easy to plot the contour of the
surface with \LaTeX.
